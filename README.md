...
> - `max_capacity` - запас хранилища для определенного отхода
> - `current_capacity` - текущая уровень заполненности хранилища определенным отходом
>
...
>
> ## Начало работы ##
> Необходимо создать основные сущности(`Вид отхода`, `Хранилище`, `Организация`). После следует определить между ними связь и указать параметры, входящие в каждую модель связи. Организация может генерировать любой вид отходов. Отходы будут храниться на счету организации, откуда их можно отправить на переработку. При достижении максимального запаса отхода в хранилище необработанный отход вернется на счет организации. Последовательность обхода хранилищей выбирается по наименьшему расстоянию.
>
> ## Тестовые данные (создаются автоматически при помощи fixtures) ##
> - ***id: name***
>
> ***Виды отходов:***
> - 1: Bio
...
> > - `crowded` - отход не обработан (все хранилища с этим типом отхода переполнены)
> > - `no_storage` - отход не обработан (отсутствуют хранилища для этого типа отхода)
